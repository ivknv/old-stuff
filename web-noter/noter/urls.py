...
urlpatterns = patterns('',
	...
	url(r'^searchajax/(?P<q>.*)/$', 'note.views.searchAjax'),
	url(r'^suggestedtags/$', 'note.views.checkSimilarityAjax'),
	url(r'^filter/(?P<pn>\d+)/$', 'note.views.filter'),
	url(r'^filter/(?P<tags>.+)/(?P<pn>\d+)/$', 'note.views.filter'),
	url(r'^filter/(?P<tags>.+)/$', 'note.views.filter'),
	url(r'^filter/$', 'note.views.filter'),
